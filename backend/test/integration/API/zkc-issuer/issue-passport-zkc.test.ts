...
test("Issue Passport ZKC for Noir", async () => {
  ...
  const sign = await wallet.signMessage(challenge.message);
  const issueReq: ZkcIssueReq = {
    sessionId: refId,
    signature: sign
  };
  ...
  const zkCred: Proved<ZkCred> = JSON.parse(issueResp.body);
  const proof = zkCred.proof[0];
  if (!proof) throw new Error(`Issued credential haven't proof`);
  a.is(proof.key, zkCred.isr.id.k);
  // @ts-ignore
  const prepared = new Preparator().prepare<number[]>(zkCred, proof.transformSchema);
  a.is(prepared.length, 182, `prepared zkc result length is not matched`)
  const hash = sha256(new Uint8Array(prepared));
  const verified = await secp256k1.verify(proof.sign, hash, `04${proof.key}`);
  a.is(verified, true);
  sinon.restore();
});
