...
const test = suite("INTEGRATION: Persona KYC service test");
...
test("handle inquiry.completed event webhook", async () => {
  ...
  // @ts-ignore
  const result = await persona.handleWebhook(req);
  ...
  a.equal(result, {
    ...
    user: {
      ...
      lastName: "SAMPLE",
      birthdate: 237945600000,
      countryCode: "US",
      document: { type: "dl", id: "I1234562" },
    },
    completed: true
  }, "Persona handle callback result is not matched");
});
