...
import { VCSignatureProver } from "../../../../../src/services/vc/vc-signature-prover.js";
...
const test = suite("INTEGRATION INTERNAL: vcSignatureProver test");

const PATH_TO_CONFIG = new URL("../../../env-config/test.env", import.meta.url);
let injector: Injector<{ didService: DIDService; vcSignatureProver: VCSignatureProver }>;
...
test("should correct sign JWS", async () => {
  const vcSignatureProver = injector.resolve("vcSignatureProver");
  const didService = injector.resolve("didService");
  const { proof } = await vcSignatureProver.sign("JsonWebSignature2020", vc!);
  a.ok(proof, "proof is undefined after signing by vcSignatureProver");
  a.ok(
    proof.jws,
    "proof.jws property is undefined after signing by vcSignatureProver"
  );
...
test("should verify credential with jws proof", async () => {
  const proofService = injector.resolve("vcSignatureProver");
