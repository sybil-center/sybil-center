...
export class HttpClient {
  ...
  >(
    ...
    const schemaName = typeof schema === "number"
      ? util.schema.toName(schema)
      : schema;
    const endpoint = new URL(util.EPs.v1(schemaName).challenge, this.issuerDomain);
  ...
  >(
    ...
    const schemaName = typeof schema === "number"
      ? util.schema.toName(schema)
      : schema;
    const endpoint = new URL(util.EPs.v1(schemaName).canIssue, this.issuerDomain);
  ...
  >(
    ...
    const schemaName = typeof schema === "number"
      ? util.schema.toName(schema)
      : schema;
    const endpoint = new URL(util.EPs.v1(schemaName).issue, this.issuerDomain);
    const resp = await fetch(endpoint, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
